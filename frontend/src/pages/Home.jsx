...
export default function Home() {
  ...
  const selectedPlaceReviews = Array.isArray(selectedPlace?.user_reviews) ? selectedPlace.user_reviews : []
  ...
  const globalReviews = useMemo(() => {
    ...
    source.forEach(place => {
      if (!Array.isArray(place?.user_reviews)) return
      place.user_reviews.forEach(review => {
        list.push({
          ...review,
    ...
    list.sort((a, b) => {
      const dateA = new Date(a.updated_at || a.created_at || a.time || 0).getTime()
      const dateB = new Date(b.updated_at || b.created_at || b.time || 0).getTime()
      return dateB - dateA
    })
  ...
  const summaryLabel = buildReviewsSummary(selectedPlace?.user_reviews_summary)
                ...
                <div className="slider-item" key={`${entry.category}-${entry.place.place_id}-${index}`}>
                  <div className="details">
                    ...
                    <div className="meta">
                      ...
                      <span className="badge muted">{entry.place?.user_reviews_summary?.count ? `${entry.place.user_reviews_summary.count} reviews` : 'Popular spot'}</span>
                    </div>
                  </div>
                ...
                <section className="place-reviews" aria-labelledby="placeReviewsTitle">
                  ...
                  <div className="reviews-stack" id="placeReviewsList">
                    {visibleReviews.length ? visibleReviews.map(review => (
                      <article key={review.review_id ?? `${selectedPlace.place_id}-${review.time ?? review.created_at ?? Math.random()}`} className="review-card">
                        ...
                        <span className="review-date">{formatDateLabel(review?.updated_at || review?.created_at || review?.time)}</span>
                        <p className="review-text">{formatReviewSnippet(review?.comment || review?.text)}</p>
                      </article>
                    )) : (
        ...
        <div className="reviews-body">
          <div className="reviews-list" id="reviewsList">
            {globalReviews.length ? globalReviews.map(review => (
              <article key={review.review_id ?? `${review.placeId}-${review.time ?? review.created_at ?? Math.random()}`} className="review-card">
                <div className="review-header">
                  <span className="review-rating"><i className="fas fa-star" aria-hidden="true"></i> {formatRating(review?.rating)}</span>
                  <span>{getReviewAuthor(review)}</span>
                </div>
                <span className="review-date">{formatDateLabel(review?.updated_at || review?.created_at || review?.time)} - {review.placeName}</span>
                <p className="review-text">{formatReviewSnippet(review?.comment || review?.text)}</p>
              </article>
            )) : (
